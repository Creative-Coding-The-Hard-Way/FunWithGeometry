...
impl Font {
        ...
        T: AsRef<str>,
    {
        ...
        let bounds =
            total_bounds.unwrap_or_else(|| Rect::new(0.0, 0.0, 0.0, 0.0));
        (tiles, bounds)
    }
...
impl From<ab_glyph::Rect> for Rect {
    fn from(ab_rect: ab_glyph::Rect) -> Self {
        Rect::new(ab_rect.min.y, ab_rect.min.x, ab_rect.max.y, ab_rect.max.x)
    }
}
